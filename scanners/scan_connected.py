"""
ConnectedGroup Scanner - M2: CDP Single Page
...
KEYWORDS = ["AI"]
LOCATION = "Hong Kong"
OUTPUT_FILE = os.path.join(os.path.dirname(__file__), "..", "candidates", "raw", f"connected_{datetime.now().strftime('%Y-%m-%d')}.json")

def scan_connected():
    print("=== ConnectedGroup Scanner ===")
    all_jobs = []
    ...
    with sync_playwright() as p:
        ...
        for kw in KEYWORDS:
            ...
            url = "https://www.connectedgroup.com/job-results?keyword=AI#/hong-kong".replace("AI", q_encoded)
            print(f"  URL: {url}")
            ...
            for card in job_cards[:15]:
                try:
                    ...
                    if link and not link.startswith("http"):
                        link = f"https://www.connectedgroup.com{{link}}"
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        json.dump({{"source": "ConnectedGroup", "date": datetime.now().isoformat(), "total_found": len(all_jobs), "jobs": all_jobs}}, f, ensure_ascii=False, indent=2)
