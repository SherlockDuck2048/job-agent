"""
PageUp Scanner - M2: CDP Single Page
URL: https://pageuppeople.com/careers/
"""
...
KEYWORDS = ["AI"]
LOCATION = "Hong Kong"
OUTPUT_FILE = os.path.join(os.path.dirname(__file__), "..", "candidates", "raw", f"pageup_{datetime.now().strftime('%Y-%m-%d')}.json")

def scan_pageup():
    print("=== PageUp Scanner ===")
    all_jobs = []
        ...
        for kw in KEYWORDS:
            ...
            for card in job_cards[:15]:
                try:
                    ...
                    company = company_el.inner_text().strip() if company_el else "PageUp"
                    
                    link_el = card.query_selector('a[href]')
                    link = link_el.get_attribute("href") if link_el else ""
                    if link and not link.startswith("http"):
                        link = f"https://pageuppeople.com{link}"
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        json.dump({"source": "PageUp", "date": datetime.now().isoformat(), "total_found": len(all_jobs), "jobs": all_jobs}, f, ensure_ascii=False, indent=2)
