"""
ZA Bank Scanner - M2: CDP Single Page
URL: https://za.group/join-us
"""
...
KEYWORDS = ["AI"]
LOCATION = "Hong Kong"
OUTPUT_FILE = os.path.join(os.path.dirname(__file__), "..", "candidates", "raw", f"zabank_{datetime.now().strftime('%Y-%m-%d')}.json")

def scan_zabank():
    print("=== ZA Bank Scanner ===")
    all_jobs = []
    ...
    with sync_playwright() as p:
        ...
        for kw in KEYWORDS:
            ...
            url = "https://za.group/join-us".replace("AI", q_encoded)
            print(f"  URL: {url}")
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        json.dump({{"source": "ZA Bank", "date": datetime.now().isoformat(), "total_found": len(all_jobs), "jobs": all_jobs}}, f, ensure_ascii=False, indent=2)
