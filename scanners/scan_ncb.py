"""
NCB Scanner - M2: CDP Single Page
URL: https://www.ncb.com.hk/nanyang_bank/html/1b3a.html
"""
...
KEYWORDS = ["AI"]
LOCATION = "Hong Kong"
OUTPUT_FILE = os.path.join(os.path.dirname(__file__), "..", "candidates", "raw", f"ncb_{datetime.now().strftime('%Y-%m-%d')}.json")
...
def scan_ncb():
    ...
    with sync_playwright() as p:
        ...
        for kw in KEYWORDS:
            ...
            url = "https://www.ncb.com.hk/nanyang_bank/html/1b3a.html".replace("AI", q_encoded)
            print(f"  URL: {url}")
            ...
            for card in job_cards[:15]:
                try:
                    ...
                    if link and not link.startswith("http"):
                        link = f"https://www.ncb.com.hk{{link}}"
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        json.dump({{"source": "NCB", "date": datetime.now().isoformat(), "total_found": len(all_jobs), "jobs": all_jobs}}, f, ensure_ascii=False, indent=2)
